...
#import "SDCircleLoadingView.h"

@interface SDCircleLoadingView ()

@property (nonatomic, strong) CAShapeLayer *shapeLayer;
@property (nonatomic, strong) CAShapeLayer *trackBackgroundShapeLayer;

@end
@implementation SDCircleLoadingView
...
+(void)startAnimations:(UIView *)view{
    ...
    [view layoutIfNeeded];
    SDCircleLoadingView *loadingView = [[SDCircleLoadingView alloc] initWithFrame:CGRectMake(view.bounds.size.width*0.5-25, view.bounds.size.height*0.5-25, 50, 50)];
...
+(void)stopAnimations:(UIView *)view{
    ...
    for (UIView *subView in view.subviews) {
        if ([subView isKindOfClass:[SDCircleLoadingView class]]) {
            SDCircleLoadingView *loadingView = (SDCircleLoadingView *)subView;
